...
def longest_subarray_of_distinct_elements(arr):
    # This set will store the distinct elements in the current window
    elements_set = set()
    # Initialize the left pointer of the window
    left = 0
    # This will keep track of the maximum length of the subarray of distinct elements
    max_length = 0
    
    for right in range(len(arr)):
        # Slide the window right, ensuring all elements are distinct        
...
#arr = [1, 5, 3, 5, 2, 3, 4, 1]
arr = [1, 5, 3, 5, 2]
print(longest_subarray_of_distinct_elements(arr))